...
mod tests {
    ...
    #[test]
    fn test_prover_first_round() {
        let random_state = vec![3u64; 100];
        ...
        let (index_pk, index_vk) = MarlinSonicInst::circuit_setup(&universal_srs, &circ).unwrap();
        let mut keys_to_constraints = BTreeMap::new();
        keys_to_constraints.insert(index_pk.circuit.deref(), std::slice::from_ref(&circ));

        let prover_state = AHPForR1CS::<_, MarlinHidingMode>::init_prover(&keys_to_constraints, rng).unwrap();
        let state = AHPForR1CS::<_, MarlinHidingMode>::prover_first_round(prover_state, rng).unwrap();
